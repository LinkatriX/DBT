namespace DBT
{
	/*public class BaseInvasion
	{
		...
		public void OnWorldUpdate()
		{
			...
			undeadipedeCount = DBTMod.BaseAI.GetNPCs(default, GRealm.UNDEADIPEDE_TYPES[0], -1, null).Length + DBTMod.BaseAI.GetNPCs(default, GRealm.UNDEADIPEDE_TYPES[1], -1, null).Length;	
		...
		public void HandleHardMobSpawn(int type)
		{
            if (Main.netMode != 1 && IsHardMob(type))
			{
				...
				if(chat != null) DBTMod.BaseUtility.Chat(chat, DBTMod.BaseConstants.CHATCOLOR_PURPLE);				
			}	
		}
		...
		public static IDictionary<int, float> SpawnPool(IDictionary<int, float> startingPool, int x, int y, Player player)
		{
			...
			}else //normal zombies
			{
				...
				if (ZONEDEV || DBTMod.BaseTile.LiquidCount(x, y, 25, 0) > 120) //floaty zombie
				{
